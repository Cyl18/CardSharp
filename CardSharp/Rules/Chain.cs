...
namespace CardSharp.Rules
{
    ...
    {
        public static (bool exists, List<Card> result) ExtractChain(this List<CardGroup> from, int chainNum, /*最少要连多少次*/
            int singleChainCardsNum, /*一个连要多少张牌*/int min /*最小的牌*/)
        {
            var matches = -1; // 满足的牌量
            ...
            foreach (var cardGroup in from.Where(cg => cg.Amount > min && cg.Amount != Constants.Cards.C2))
            {
                ...
                if (chainNum == matches)
                {
                    ...
                    foreach (var amount in amounts)
                        for (var i = 0; i < singleChainCardsNum; i++)
